...
export default new class extends ScrapingSite<void, any> {
  ...
  protected fetch(u: void): any {
    const next = Array.from(document.querySelectorAll("#searchResults > p.pager > span.next > a"))
      .map(e => (e as HTMLAnchorElement).href)
    const cards = Array.from(document.querySelectorAll("#searchResults > div > table > tbody > tr > td > h4 > a"))
      .map(e => (e as HTMLAnchorElement).href)
    return { next, cards }
  }
